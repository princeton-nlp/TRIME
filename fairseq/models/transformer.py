...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.knn_keytype = args.knn_keytype if args.knn_keytype is not None else 'last_ffn_input'
    ...
    def extract_features(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.decoder_layerdrop):
                ...
                if get_ffn_inp:
                    ...
                    knn_embs['last_ffn_input'] = layer_attn[1]
        ...
        if self.knn_keytype == 'model_output':
            knn_emb = x
        knn_embs['model_output'] = x

        # T x B x C -> B x T x C
        x = x.transpose(0, 1)

        if self.knn_keytype in ['last_ffn_input', 'model_output', 'separated_head']:
            # return x, {'attn': [attn], 'inner_states': inner_states, self.knn_keytype: knn_emb}
            return x, {'attn': [attn], 'inner_states': inner_states, **knn_embs}
