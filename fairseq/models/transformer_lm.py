...
def transformer_lm_wiki103_small(args):
    ...
    args.no_decoder_final_norm = getattr(args, 'no_decoder_final_norm', True)
    args.tie_adaptive_proj = getattr(args, 'tie_adaptive_proj', True)
    args.knn_type = getattr(args, 'knn_type', 'last_ffn_input')
    transformer_lm_big(args)

# 7M model without using adaptive softmax
@register_model_architecture('transformer_lm', 'transformer_lm_wiki103_small_sm')
def transformer_lm_wiki103_small_sm(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 8)
    ...
    args.activation_dropout = getattr(args, 'activation_dropout', 0.1)
    args.no_decoder_final_norm = getattr(args, 'no_decoder_final_norm', False)
    args.knn_type = getattr(args, 'knn_type', 'model_output')
    args.learn_mem_weight = getattr(args, 'learn_mem_weight', False)
    transformer_lm_big(args)

# 150M model (the same architecture as Transformer-XL)
...
# the model for enwik8 experiments
