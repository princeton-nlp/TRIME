...
def my_gather(t):
    # Do not need to gather tensors from other devices
    return t, (0, t.shape[0])

@register_criterion('trime_long_loss_same_device')
class TrimeLongLoss_same_device(FairseqCriterion):
    """
    *******************************************************************************
    *** We assume all memories are in the same gpu device in this loss function ***
    *** Otherwise, please use the default TrimeLongLoss instead                 ***
    *******************************************************************************
    ...
    def compute_in_bacth_logits(self, reps, labels, only_cross_sent=False):
        B = reps.shape[0]
        L = reps.shape[1]
        ...
        temp_mask = torch.ones((bsz, bsz), device=reps.device)
        inbatch_mask = torch.triu(temp_mask, diagonal=0)
        inbatch_mask = inbatch_mask[:, -inbatch_logits.shape[1]:]
        inbatch_mask = inbatch_mask * -10000.0
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if self.training and (self.args.cross_sent_ratio is not None):
            in_batch_logits, in_batch_labels = self.compute_in_bacth_logits(reps, orig_target, 
                        only_cross_sent=(random.random() < self.args.cross_sent_ratio))
        else:
            in_batch_logits, in_batch_labels = self.compute_in_bacth_logits(reps, orig_target)
