...
def my_gather(t):
    """
    Gather tensor t from all devices
    """
...
def compute_in_bacth_logits(reps, labels, only_cross_sent=False):
    B = reps.shape[0]
    L = reps.shape[1]
    ...
    # each token can access all tokens in other segments
    inbatch_mask = torch.zeros(inbatch_logits.shape, device=reps.device).type_as(reps)
    # access within the same segment is defined by local_mask
...
@register_criterion('trime_ext_loss')
class TrimeExtLoss(FairseqCriterion):
    """
    This is an implementation of the Trime loss. 
    In this function, we use previous tokens in the same segment and all tokens in the same batch to construct working memory.
    """
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if self.training and (self.args.cross_sent_ratio is not None):
            in_batch_logits, in_batch_labels = compute_in_bacth_logits(reps, orig_target, only_cross_sent=(random.random() < self.args.cross_sent_ratio))
        else:
