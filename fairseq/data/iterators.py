...
class EpochBatchIterator(EpochBatchIterating):
    ...
    def __init__(
        ...
        num_workers=0, epoch=0, predefined_batches=False, keep_order=False, num_comb_shards=1
    ):
    ...
    def _get_iterator_for_epoch(self, epoch, shuffle, fix_batches_to_gpus=False, offset=0):
        ...
        def shuffle_batches(batches, seed):
            if not self.predefined_batches:
                ...
                # shuffle num_shards batches together. make sure predefined batches are in the same train batch
                grouped_batches = []
                for i in range(0, len(batches), self.num_shards):
                    grouped_batches.append(batches[i:i+self.num_shards])
                with data_utils.numpy_seed(seed):
        ...
        if self._supports_prefetch:
            ...
            if not self.keep_order:
                ...
                batches = list(SequentialShardedIterator(
                    batches, self.num_shards, self.shard_id, fill_value=[], num_comb_shards=self.num_comb_shards
                ))
...
class SequentialShardedIterator(object):
    """A sharded wrapper around an iterable, padded to length.
    ...
    def __init__(self, iterable, num_shards, shard_id, fill_value=None, num_comb_shards=1):
        if shard_id < 0 or shard_id >= num_shards:
            raise ValueError('shard_id must be between 0 and num_shards')

        assert(num_shards % num_comb_shards == 0)
        num_shards = num_shards // num_comb_shards
        self._sharded_len_combined = len(iterable) // num_shards
        if len(iterable) % num_shards > 0:
            self._sharded_len_combined += 1

        self._sharded_len = self._sharded_len_combined // num_comb_shards
        if self._sharded_len_combined % num_comb_shards > 0:
            self._sharded_len += 1
