...
def add_optimization_args(parser):
    ...
    group.add_argument('--ce-warmup-epoch', default=0, type=int, help='how many epochs to use CE loss only')
    group.add_argument('--cross-sent-ratio', default=None, type=float, help='how much time should the model only retrieves from cross-sent tokens (disabling local memory)')
...
def add_eval_lm_args(parser):
    ...
    add_common_eval_args(group)
    # fmt: off
    group.add_argument('--eval-file', default=None, type=str,
                       help='Save the evaluation results in this file')
    ...
    group.add_argument('--bisearch-l', type=float, default=0.01)
    group.add_argument('--bisearch-r', type=float, default=15.00)
    group.add_argument('--bisearch-step', type=int, default=11)

    group.add_argument('--mode', type=str, default='gn')
    group.add_argument('--use-local', action='store_true', help='whether use local memory')
    group.add_argument('--use-long', action='store_true', help='whether use long-term memory')
    group.add_argument('--use-external', action='store_true', help='whether use external memory (datastore)')
    group.add_argument('--use-interp', action='store_true', help='whether use the linear interpolation at the end')
    # fmt: on
...
def add_contrastive_args(parser):
    group = parser.add_argument_group("Contrastive learning configuration")
    # fmt: off
    group.add_argument('--dist', default='l2', type=str,
                       help='distance function used in contrastive learning')
    group.add_argument('--temp', default=1.0, type=float,
                       help='temperature term used in contrastive learning')
    # fmt: on
