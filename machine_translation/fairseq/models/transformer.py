...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def forward(
        ...
        if self.use_knn_datastore:
            ...
            if self.knn_lambda_type == "trainable" and self.knn_k_type == 'trainable':
                ...
                elif self.knn_lambda_type == 'trime':
                    knn_temperature *= math.sqrt(last_hidden.shape[-1])
                    logits = x[:, -1:, :]
                    norm_t = torch.logsumexp(logits, dim=-1, keepdim=True) # [B, S, 1]
                    norm_c = torch.logsumexp(knn_dists / knn_temperature, dim=-1, keepdim=True) # [B, S, 1]
                    norm_tpc = torch.logsumexp(torch.stack((norm_t, norm_c), dim=-1), dim=-1)
                    knn_lambda = torch.exp(norm_c - norm_tpc)
                else:
    ...
    def extract_features_scriptable(
        ...
        return x, {"attn": [attn], "inner_states": inner_states, "knn_embed": x}
