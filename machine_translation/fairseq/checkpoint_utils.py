...
def save_checkpoint(args, trainer, epoch_itr, val_loss):
    ...
    checkpoint_conds["checkpoint_{}_{}{}.pt".format(epoch, updates, suffix)] = (
        ...
        and updates >= args.validate_after_updates  # add by  ,
        and args.keep_interval_updates > 0  # add by  , not automatically save model
    )
    checkpoint_conds["checkpoint_best{}.pt".format(suffix)] = val_loss is not None and (
            not hasattr(save_checkpoint, "best")
            or is_better(val_loss, save_checkpoint.best)
    ) and args.keep_best_checkpoints != 1
    # add by  
    # this maybe redundant because we will save the model based on the metric by the lines of code below. so we remove it when
    ...
    if args.keep_interval_updates > 0:
